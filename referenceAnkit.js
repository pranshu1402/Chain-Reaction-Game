...
*/
	...
	var t1Grad = t1Ctx.createRadialGradient(
		radius,
		radius,
		0,
		radius,
	...
	var tmpGrad = tmpCtx.createRadialGradient(
		radius,
		radius,
		0,
		radius,
	...
	function initializeGrid(rows, columns) {
		//grid[r][c] = top left cordinates for r,c box
		var x1 = 10;
		var y1 = 10;
	...
	function DrawGrid(rows, columns, x, y) {
		...
		return Math.min(
			HorizontalDistanceBetween2Lines,
			VerticalDistanceBetween2Lines
		);
	}

	function intializeBalls(rows, columns) {
		//Status  = 0 means box is empty
		for (c = 0; c < columns; ++c) {
			balls[c] = [];
			for (r = 0; r < rows; ++r) {
			...
			for (c = 0; c < columns; ++c) {
				if (
					...
					if (balls[c][r].Status >= 2) {
						...
						balls[c][r].Status -= 2;
						if (balls[c][r].Status == 0) balls[c][r].player_id = 0;
						flag = true;
						//console.log("Status == 4 found at ",r," ",c);
					}
				} else if (
					((r == 0 || r == Rows - 1) && c > 0 && c < Columns - 1) ||
					((c == 0 || c == Columns - 1) && r > 0 && r < Rows - 1)
				) {
